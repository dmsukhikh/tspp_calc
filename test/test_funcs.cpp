...
TEST(TestFuncs, TestDet)
{
    ...
    MatrixGeneric a = {{1, 4, 0, 12},{-1, -2, 1, 1},{0, 1, 1, -1},{7, 0, 1, 2}};
    EXPECT_NEAR(a.det(), 325, 10e-6);
    EXPECT_EQ((3 * a).det(), 325 * 81);

    MatrixGeneric b = {{1, 7, 0},{-1, -2, 0},{1, 3, 5}};
    EXPECT_NEAR(b.det(), 25, 10e-6);

    MatrixGeneric rect = {{1, 3, 0, 2}, {1, 0, 1, 2}, {1, 2, 3, 1}};
    EXPECT_THROW(rect.det(), matrix_bad_det);

    MatrixGeneric<int> empty;
    EXPECT_NEAR(empty.det(), 1, 10e-6);

    MatrixGeneric diag = {{2, 3, 2},{0, 1, 2},{0, 0, 4}};
    EXPECT_NEAR(diag.det(), 2*1*4, 10e-6);

    MatrixGeneric zerodet = {{1, 1, 2, 2},{0, 1, 0, 0},{0, 0, 1, 0},{1, 3, 2, 2}};
...
TEST(TestFuncs, TestRk)
{
    ...
    MatrixGeneric b = {{1, 4, 0, 12},{-1, -2, 1, 1},{0, 1, 1, -1},{7, 0, 1, 2}};
    EXPECT_EQ(b.rk(), 4);

    MatrixGeneric c = {{1, 0, 0, 0, 12}, {0, 1, 0, 11, -11}, {0, 0, 1, -2, -5}};
    EXPECT_EQ(c.rk(), 3);
    
    MatrixGeneric d = {{1, 0},{0, 1},{1, 1},{0, 0},{1, 1}}; 
    EXPECT_EQ(d.rk(), 2);

    MatrixGeneric e = {{1, 1, 1}, {0, 1, 0}, {2, 5, 2}};
    EXPECT_EQ(e.rk(), 2);
...
{
    MatrixGeneric a = {{1, 4, 3, 2}, {1, 4, 5, 6}, {0, 1, 4, 0}};
    auto b = a.transpose();
...
{
    MatrixGeneric bad_1 = {{1, 2, 3, 4, 5}},
                  bad_2 = {{1, 2},  {2, 4}};
    EXPECT_THROW(bad_1.inverse(), matrix_bad_inverse);
    EXPECT_THROW(bad_2.inverse(), matrix_bad_inverse);

    MatrixGeneric good_1 = {{1, 2, 3}, {4, 5, 6}, {7, 8, 8}},
                  good_2 = {{1, 4, 0, 12},{-1, -2, 1, 1},{0, 1, 1, -1},{7, 0, 1, 2}};
    
    MatrixGeneric out_1 = good_1 * good_1.inverse(),
                  out_2 = good_2 * good_2.inverse();

    for (auto &testCase :
         {std::reference_wrapper(out_1), std::reference_wrapper(out_2)})
    {
